...
def analyze_missing_players(home_id, away_id, game_id):
    ...
    for inj in injuries:
        ...
        info = {
            ...
            "position": position,
            "goals_impact": player_stats.get("goals_impact", 0.1),
    ...
    def generate_team_analysis(missing_list, positions, side_name):
        if not missing_list:
        ...
        for p in missing_list:
            ...
            text += f"     → Замена: {p['replaced_by']}\n"
            
            if p["position"] == "G":
                main_goals = main_stats.get("goals_conceded_avg") or 0
                backup_goals = backup_stats.get("goals_conceded_avg") or 0
                if backup_goals > 0 and main_goals > 0:
                    if backup_goals > main_goals * 1.15:
                        text += f"     ⚠️ Пропускает больше ({backup_goals} vs {main_goals}). ХУЖЕ.\n"
                    elif backup_goals < main_goals * 0.85:
                        text += f"     ✅ Пропускает меньше ({backup_goals} vs {main_goals}). ЛУЧШЕ!\n"
                    else:
                        text += f"     ≈ Примерно равен ({backup_goals} vs {main_goals}).\n"
                else:
                    text += f"     ⚠️ Нет данных для сравнения.\n"
            
            elif p["position"] == "D":
                main_err = main_stats.get("errors_leading_to_goal") or 0
                backup_err = backup_stats.get("errors_leading_to_goal") or 0
                if backup_err > main_err:
                    text += f"     ⚠️ Ошибается чаще ({backup_err} vs {main_err}). ХУЖЕ.\n"
                elif backup_err < main_err:
                    text += f"     ✅ Ошибается реже ({backup_err} vs {main_err}). ЛУЧШЕ!\n"
                else:
                    text += f"     ⚠️ Нет данных для сравнения.\n"
            
            elif p["position"] == "F":
                main_g = main_stats.get("goals_scored") or 0
                backup_g = backup_stats.get("goals_scored") or 0
                main_mp = max(main_stats.get("matches_played", 1) or 1, 1)
                backup_mp = max(backup_stats.get("matches_played", 1) or 1, 1)
                main_avg = round(main_g / main_mp, 2)
                backup_avg = round(backup_g / backup_mp, 2)
                if backup_avg > 0 and main_avg > 0:
                    if backup_avg < main_avg * 0.7:
                        text += f"     ⚠️ Забивает реже ({backup_avg} vs {main_avg}). ХУЖЕ.\n"
                    elif backup_avg > main_avg * 1.3:
                        text += f"     ✅ Забивает чаще ({backup_avg} vs {main_avg}). ЛУЧШЕ!\n"
                    else:
                        text += f"     ≈ Примерно равен ({backup_avg} vs {main_avg}).\n"
                else:
                    text += f"     ⚠️ Нет данных для сравнения.\n"
        ...
        text += f"\n  📊 ИТОГ: "
        if d_count >= 2:
            text += f"Выбыли {d_count} защитника. Центр обороны разрушен. Ждите голов!"
        elif d_count == 1 and g_count >= 1:
            text += f"Выбыл защитник и вратарь. Оборона ослаблена."
        elif d_count == 1:
            text += f"Выбыл 1 защитник. Оборона ослаблена."
        elif g_count >= 1:
            text += f"Выбыл вратарь. Смотри сравнение выше."
        elif f_count >= 1:
            text += f"Выбыл нападающий. Атака ослаблена."
        else:
            text += f"Потери есть, но не критические."
        
        return text
    
    home_full_text = generate_team_analysis(missing_home, home_positions, "Хозяева")
    away_full_text = generate_team_analysis(missing_away, away_positions, "Гости")
    ...
    full_analysis = f"{home_full_text}\n\n{away_full_text}\n\n🧠 ОБЩИЙ ВЕРДИКТ: "
    if total_penalty > 0.5:
        full_analysis += f"Оборона хозяев ослаблена (+{total_penalty} к голам гостей). ТБ 2.5 рекомендуется."
    elif total_penalty < -0.5:
        full_analysis += f"Оборона гостей ослаблена ({total_penalty} к голам хозяев). ТБ 2.5 рекомендуется."
    elif total_penalty > 0.2:
        full_analysis += f"Небольшое преимущество гостей."
    elif total_penalty < -0.2:
        full_analysis += f"Небольшое преимущество хозяев."
    else:
        full_analysis += f"Составы равнозначны."
...
def collect_player_stats(league_id, year):
    ...
    for gs in games:
        ...
        for ps in full.get("playerStats") or []:
            ...
            if name not in players:
                players[name] = {"matches": 0, "goals": 0, "errors": 0, "yc": 0}
            players[name]["matches"] += 1
            players[name]["goals"] += ps.get("goalsTotal") or 0
            players[name]["errors"] += ps.get("errorsLeadingToGoal") or 0
            players[name]["yc"] += ps.get("cardsYellow") or 0
        time.sleep(0.2)
    saved = 0
    for name, s in players.items():
        ...
        try:
            ...
            if existing.data:
                supabase.table("player_stats").update(row).eq("player_name", name).execute()
            else:
                supabase.table("player_stats").insert(row).execute()
            saved += 1
        except Exception as e:
...
def generate_verdicts(game, statistics, odds_list, referee_name, home_id, away_id, home_name, away_name, year):
    ...
    missing_analysis = analyze_missing_players(home_id, away_id, game_id) if game_id else {"total_penalty": 0, "missing_home": [], "missing_away": [], "full_analysis": ""}
    ...
    base_analysis = {
        ...
        "full_analysis": missing_analysis.get("full_analysis", "")
    }
    ...
    if "fouls" in lines:
        ...
        if hf and af:
            preds.append(hf + af + lineup_penalty * 0.5)
            weights.append(0.35)
...
def main():
    print("🚀 ЗАПУСК СБОРЩИКА (14 лиг, 3 сезона) + УМНЫЕ СОСТАВЫ, ТРАВМЫ, GLICKO, ФОРМА, ИГРОКИ")
    print("=" * 55)
    ...
    for league_id, league_name in TOP_LEAGUES.items():
        ...
        for year in SEASONS:
            ...
            # Статистика игроков
            player_count = supabase.table("player_stats").select("id", count="exact").eq("league_id", league_id).eq("year", year).execute()
            if not player_count.count:
